...
export class DeepDive {
  ...
  async ask(question) {
    ...
    try {
      ...
      // Validate response - handle both standard and mobile backend formats
      if (!response.response && !response.success) {
        throw new Error('No response data received');
      }
      
      // Get the actual response text
      const responseText = response.response || response.message || 'No response received';
      
      // Display response
      this._displayResponse(selectedText, question, responseText);
