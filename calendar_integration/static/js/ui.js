...
export const ui = {
  ...
  openEventModal (defaults = {}) {
    ...
    // Add eventType dropdown if not present
    if (!form.querySelector('[name="eventType"]')) {
      ...
      `;
      // Insert after title
      const titleGroup = form.querySelector('input[name="title"]').closest('.form-group');
      titleGroup.insertAdjacentElement('afterend', eventTypeGroup);
    }
    ...
    form.description.value   = defaults.description   ?? '';
    form.location.value      = defaults.location      ?? '';
